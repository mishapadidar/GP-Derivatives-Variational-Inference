...
class DirectionalGradVariationalStrategy(_VariationalStrategy):
    ...
    def forward(self, x, inducing_points, inducing_values,variational_inducing_covar=None, **kwargs):
        ...
        # repeat the set of shared inducing directions and values
        num_induc = inducing_points.size(-2)
        num_directions = inducing_directions.size(-2)
        inducing_directions = inducing_directions.repeat(num_induc,1)
        # make the last p inducing values the directional derivatives
        dds = inducing_values[-num_directions:].repeat(num_induc)
        iv  = torch.zeros((num_induc,num_directions+1))
        iv[:,0] = inducing_values[:-num_directions]
        iv[:,1:] = dds.reshape((num_induc,num_directions))
        iv = iv.flatten()
        inducing_values = iv
        if torch.cuda.is_available():
            inducing_values = inducing_values.cuda()
        ...
        middle_term = torch.zeros((num_induc*(num_directions+1),num_induc*(num_directions+1)))
        if torch.cuda.is_available():
            middle_term = middle_term.cuda()
